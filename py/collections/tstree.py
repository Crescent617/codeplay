class TSTreeNode(object):
    ...
    def __init__(self, key, value, char=None, low=None, eq=None, high=None):
        self.key = key
        self.value = value
        self.char = char
        self.low = low
...
class TSTree(object):
    ...
    def _set(self, node, chars, key, value):
        ...
        if not node:
            ...
            node = TSTreeNode(None, None, char=next_char)
    ...
    def set(self, key, value):
        self.root = self._set(self.root, key, key, value)
    ...
    def _list(self, node, intend=0):
        ...
        if node and node.char:
            ...
            if node.low:
                print(' ' * intend, '>', end='')
                self._list(node.low, intend+2)
            if node.eq:
                print(' ' * intend, '=', end='')
                self._list(node.eq, intend+2)
            if node.high:
                print(' ' * intend, '<', end='')            
                self._list(node.high, intend+2)    
